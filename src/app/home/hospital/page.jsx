            ...
            <div className="w-full max-w-6xl mx-auto">
                <div className="grid grid-cols-4 gap-x-8 gap-y-8">
                    {hospitalLists.map((item, index)=>(
                        <div key={index}>
                            <img src="/sample.svg" className="w-[365px] h-[262px] object-cover rounded-lg mb-3" alt="" />
                            <h3 className="text-[20px] text-[#327CEC] tracking-wide">{item?.name}</h3>
                            <ul className="my-2 text-black flex flex-wrap gap-x-2">
                                {item.department?.map((dep)=>(
                                    <li>#{dep}</li>
                                ))}
                            </ul>
                            <button className="text-white fond-sm flex gap-x-3 items-center">See Details <img src="/rightArrow.svg" alt="" /> </button>
                        </div>
                    ))}

                </div>
                <div className="flex justify-center w-full mt-8">
            <button className="bg-[#327CEC] mb-10 px-8 py-3 text-sm rounded-full tracking tracking-wider">Load More</button>
                </div>
            </div>
        </section>
    );
